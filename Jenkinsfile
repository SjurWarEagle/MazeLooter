...
pipeline {
  ...
  triggers {
    GenericTrigger(
      ...
      token: 'tokenMazeLooter'+ BRANCH_NAME,
      tokenCredentialId: '',
  ...
  environment {
    PRODUCT_NAME = "MazeLooter"
    NPM_CONFIG_PROGRESS = false
    NPM_CONFIG_SPIN = false
    NORMALIZED_BRANCH_NAME = env.BRANCH_NAME.replaceAll('[^a-zA-Z\\d\\s:]', '_')
    DOCKER_IMAGE_NAME = "maze-looter" + "-branch_" + "${env.NORMALIZED_BRANCH_NAME}"
  }
          ...
          docker.image("${env.DOCKER_IMAGE_NAME}").inside() {
            withEnv([
              ...
              */
              ...
              sh 'cp -vr dist/maze-looter/* /builds/${PRODUCT_NAME}/'
            }
          }
        }
      }
